...
if [[ "$answer" =~ ^[Yy]$ ]]; then
	...
	cp -f config/debian/sources.list /etc/apt/sources.list
	sudo apt update -y
        sudo apt-get clean
    ...
    # normal software
    sudo apt install -y nano dpkg dbus-x11 htop nitrogen gedit desktop-base \
    rofi lightdm firefox tigervnc-standalone-server tigervnc-common \
